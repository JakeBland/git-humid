...
def process_single_ascent(source, station_number, time, dtype, filter_dic, flag, lead_time = 0, kind = 'linear'):
    """
    Produce list of filtered variables which can be calculated without derivatives from given data
    ...
    """
    variables = ['air_pressure', 'air_temperature', 'air_potential_temperature', 'dew_point_temperature', 'specific_humidity',
                 'altitude', 'mass_fraction_of_cloud_ice_in_air', 'mass_fraction_of_cloud_liquid_water_in_air']
    ...
    # calculate variables such that all profiles will have, as a minimum, fields of altitude, p, T, theta, q, RHi and RHw
    if dtype == 'UKMO':
    ...
    cubelist_smooth.append(iris.cube.Cube(trop_alt, standard_name = 'tropopause_altitude', units = 'm',
